...
class CharNet(nn.Module):
    ...
    def init_hidden(self, batch_size):
        return (tr.zeros(1, batch_size, 64, device='cpu'),
                tr.zeros(1, batch_size, 64, device='cpu'))
...
class PredNet(nn.Module):
    ...
    def train(self, data_loader, optim):
        ...
        for i, batch in enumerate(data_loader):
            ...
            past_traj = past_traj.float().cpu()
            curr_state = curr_state.float().cpu()
            target_action = target_action.long().cpu().squeeze(-1)
            target_consume = target_consume.long().cpu().squeeze(-1)
            target_sr = target_sr.float().cpu()
